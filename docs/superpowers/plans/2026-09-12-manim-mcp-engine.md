...
| 20 | Task 20（对话正文内嵌动画） | 计划把 `previewMarkdown` 设计成「指向渲染产物的同源地址」，实现时第一版取 `/D:/...`、第二版取「以渲染根目录名开头」的 `/<root>/<run>/out/S.gif`。**两版都 404**：前端 markdown 只接受 `http(s)://` 或以 `/` 开头且不以 `//` 开头的目标，而 Host 用 `node:path.resolve` 解析——对以 `/` 开头的实参，`resolve` **丢弃 cwd 的目录、只取它的盘符**。实测：`resolve('D:\\proj','/renders/x')` → `D:\\renders\\x`（落空）；`resolve('D:\\proj','/D:/x')` → `D:\\D:\\x`（落空）；`resolve('D:\\proj','/myprogram/dshplugin/renders/x')` → `D:\\myprogram\\dshplugin\\renders\\x`（命中）。也就是说**只有盘符是有效的，目录部分完全被忽略** | `preview_markdown` 改为发出「**去掉盘符的完整绝对路径**」（`D:\\a\\b.gif` → `/a/b.gif`），并把该规则抽成可直接断言的纯函数 `rooted_reference`；产物不在渲染根内、或绝对路径没有盘符（UNC）时返回 `None`。`install.ps1` 相应地把 `fs-sandbox.cwd` 钉到**与渲染根同盘**的 `render root 的父目录`——它唯一的职责就是提供盘符。新增 `tests/manual/probe_file_api.mjs`：用本机 browser-session secret 签一个真实 cookie，对运行中的 Host 发 `HEAD /api/file`，实测新形式 200（`image/gif`，字节数与 `previewBytes` 逐字节一致）、两种旧形式 404、缺 cookie 401 |
| 21 | Task 20 | 旧口径下 `pin-fs-cwd` 步的 `changed` 只检查 `cwd:` 那一行在不在，于是**只改块上方注释的重装会报 `changed: false`，而该次运行其实重写了整个块**——一个「无需变更」的计划配上被改掉的文件，陈旧注释就这样活过重装 | `changed` 改为按**整块逐字比对**（`build_fs_overlay_block(targets) not in patch_text`），新增回归测试 `test_plan_install_notices_a_stale_comment_on_an_otherwise_correct_overlay` 构造「cwd 正确但注释陈旧」的状态并断言 `changed is True` |
...
> 第 20 条的教训：**「路径看起来对」和「路径解析得到」是两件事。** 前后两版都是在读完文档、推理出「应该能行」之后写下的，两次都 404；真正定案的是一行 `node -e` 的 `resolve` + `existsSync` 对照实验。凡是把路径交给另一个运行时解析的地方，都必须用那个运行时本身验一次，不能用自己的直觉代跑。
