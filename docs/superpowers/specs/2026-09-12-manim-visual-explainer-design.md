...
- 一个 DSH 原生插件（Host + Client），提供「动画库」面板（见第 19 节）。
- 一个 Skill，规定「什么时候画、怎么画才有效」。
- 幂等的安装/卸载脚本与完整中文文档。

> **实现变更（晚于本方案的实测结论，优先于下文对应文字）**
>
> 本文最初把「动画库」设计成**左侧栏常驻图标 + 全尺寸中央面板**（`sidebar.panellist` + `main` + `layout.selectPanel`）。实际实现与验收后，按用户要求改成：
>
> | | 原方案 | 实际实现 |
> |---|---|---|
> | 入口 | 左侧栏常驻图标 | **只有会话头部右上角一个图标按钮**（`conversation.session.header.utilities`） |
> | 呈现 | `layout.selectPanel` 切主区 → 全尺寸中央面板 | 注册**页面型右侧栏 tab**（`sidebarRightTabs.register({id,kind,priority,title})` + `sidebar.right.pane.tab`），按钮调 `sidebarRight.openTab(kind)`，右栏**在旁展开**，不覆盖页面 |
> | `main` / `sidebar.panellist` | 二者都用 | **都不再注册** |
> | 变更原因 | — | 用户：「只和侧边栏一样，只是在侧边展开，不要覆盖整个页面」 |
>
> 另有两条实现要点是本文没有的：tab **type** 必须在 `apply` 里经 `ctx.effect` **先注册**（`openTab` 对未注册的 kind 会抛，而按钮可能早于右栏座位被点击）；本插件**不贡献 `guide` 条目**，因为默认页由 guide 条目数决定，加一条会改变所有用户首次展开右栏看到的内容。
>
> 下文第 19 节与 5.x/15.x 各处仍保留原方案的叙述作为设计记录，**冲突时以本表为准**。正文内嵌动画那条通道（第 12 节通道 B）才是用户看到动画的**主通道**，面板只是回看入口。
...
6. 断言：面板的搜索 / 类型筛选 / 排序 / 显示失败开关都生效；面板**没有任何写操作**（见 §19.5.1）。
...
6. 对话内动画正常内嵌显示。
7. 动画库面板：列表与 `renders\` 实际内容一致，GIF 缩略图与 `<video controls>` 都能加载，搜索/筛选/排序生效，且面板**不发出任何写请求**。
...
- **点击卡片 → 详情视图**（在同一面板内切换，不新开路由）：
  - 视频播放器：`<video controls>` 播 MP4（可暂停、拖进度——这是 GIF 做不到的）；没有 MP4 时退回 GIF/海报帧。
  - 元信息：类型、状态、画质、时长、渲染耗时、生成时间、runId、预览与视频的绝对路径。
  - 操作：**在右侧打开**（把这个 run 的 GIF/WebP/PNG 交给 DSH 自带的右侧栏文档预览器）与**复制路径**（MP4 优先，退回预览产物）。
    - 走 `ctx.sidebarRight.openResource("dsh-resource://file/absolute/<path>")`，并先由 `ctx.layout.openRightbar(true, false)` 展开抽屉（`openResource` 是否顺带展开不属于它的契约，显式调用才不会开出看不见的 tab）。
    - **只交预览产物，不交 MP4**：自带预览器的 MIME 表里有 `gif`/`webp`/`png`，没有 `video/mp4`；交 MP4 会开出一个渲染不出内容的 tab。完整视频仍由面板内的 `<video controls>` 承担。
    - 两个服务用 `ctx.get()` **探测**而非 `inject` 硬依赖：缺服务时少一个按钮，而不是整个面板不可用。这条与 §19.6 的解耦原则一致。
- **顶部工具条**：关键词搜索（标题 + 类型）、按类型筛选、按生成时间或时长排序、显示失败项开关、手动刷新。
- **空态**：「还没有动画。去问一个值得画的问题吧。」
- **错误态**：`index.json` 读取失败或损坏时，显示**明确原因**（含 HTTP 状态码）、**它尝试读取的绝对路径**、以及「重试」按钮，并提示对话内的动画不受影响。

### 19.5 实现约束

- 单个 `lib/client.js`，手写 `window.__ModuleLoader__.load({id, factory})` 形式，**无构建步骤**（与 `dsh-cross-session-modal` 同款）。
- 样式用注入 `<style data-plugin-css=...>` 的方式，类名带前缀；颜色**只能**走 `--dsw-*` 变量以跟随明暗主题（有一处硬编码 hex 就会在浅色主题下变成深色叠深色）。
- `apply(ctx)` 内所有注册都必须可回收：用 `ctx.slots.inject(name, () => ctx.slots.register(...))`，返回值交由 Cordis 管理生命周期。
- `exports.inject = ["slots"]`（面板不需要会话或 remote 服务）。
- 面板不做轮询：**仅在挂载与手动刷新时**读取 `index.json`。
- **逻辑与视图分离**：数据变换（路径/URL/时间/排序/筛选/取索引）与元素树构造（`galleryView(state, actions)`）都是纯函数，React 那一层只负责把 state 接进去。这是让面板能在 Node 里离线测试的前提。

### 19.5.1 只读边界（重要，这是有意的设计决定）

面板**不做删除，也不做重渲染**。

原因是技术性的：手写打包插件**没有 client→host 调用通道**。DSH 的 client→host 调用走的是 Typert Gateway 生成的 Remote 贡献（见 `dsh-api-remotes`），那需要完整的 TypeScript/tsdown 构建管线与声明合并；手写插件可用的桥只有 host→client 的转发事件白名单。删除要写文件系统、重渲染要调用 MCP，两者都必须由 Host 侧发起。

因此这两件事改由 **MCP 侧的 `runs` 工具**承担：用户对模型说「把那张图删了」，模型调用 `runs action=delete`。面板只提供「复制路径」，让用户在资源管理器里自行处理。

这条边界是 Plan 2 对本文档的**范围缩减**，理由如上。
